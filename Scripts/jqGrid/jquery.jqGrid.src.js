...
(function ($) {
		...
		_useProperties = true;
			...
			findSortKey;
                    ...
                    if (type == 'float' || type == 'number' || type == 'currency' || type == 'numeric') {
                        ...
                        findSortKey = function ($cell) {
                            if (!$cell) { $cell = ""; }
                            if ($cell == "Not Assigned") { $cell = ""; }
                            return $.trim(String($cell).toUpperCase());
                        };
                    }

                    for (var di = 0; di < data.length; di++) {
                        ab = by !== "" ? $.jgrid.getAccessor(data[di], by) : data[di];
                        if (ab === undefined) { ab = ""; }
                        ab = findSortKey(ab, data[di]);
                        _sortData.push({ 'vSort': ab, 'index': di });
                    }
    ...
    $.fn.jqGrid = function (pin) {
        ...
        return this.each(function () {
            ...
            var ts = this, grid = {
                ...
                dragStart: function (i, x, y) {
                    this.resizing = { idx: i, startX: x.clientX || x.originalEvent.pageX, sOL: y[0] };
                ...
                dragMove: function (x) {
                    var currentX = x.clientX || x.originalEvent.pageX;

                    if (this.resizing) {
                        var diff = currentX - this.resizing.startX,
		...
		isMSIE = $.browser.msie ? true : false,
            // The following is a fix for Chrome < version 19
            //isSafari = $.browser.webkit || $.browser.safari ? true : false;
        isSafari = ($.browser.webkit || $.browser.safari) && parseFloat($.browser.version) < 536.11 ? true : false;
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata) {
		    ...
		    result += (clas !== undefined ? (" class=\"" + clas + "\"") : "") + ((cm.title && tv) ? (" title=\"" + $.jgrid.stripTitleHtml(tv) + "\"") : "");
		...
		addCell = function (rowId, cell, pos, irow, srvr) {
		    ...
		    //HD: we have cases where we want to display XML values and show them (backing data is xml) and where
		    //we've actually formatted the data to replace the cell with html.  For now just checking the <i and <span tags
		    var isHtmlTag = v.indexOf("<i") > -1 || v.indexOf("<span") > -1;
		    return "<td role=\"gridcell\" " + prp + ">" + (isHtmlTag ? v : v.replace("<", "&lt;").replace(">", "&gt;")) + "</td>";
		},
		...
		addLocalData = function () {
		    var st, fndsort = false, cmtypes = {}, grtypes = [], grindexes = [], srcformat, sorttype, newformat, c;
		    if (!$.isArray(ts.p.data)) {
		        return;
		    }
		    var grpview = ts.p.grouping ? ts.p.groupingView : false;

		    for (c = 0; c < ts.p.colModel.length; c++) {
		        var current = ts.p.colModel[c];
		        sorttype = current.sorttype || "text";
		        if (sorttype == "date" || sorttype == "datetime") {
		            if (current.formatter && typeof (current.formatter) === 'string' && current.formatter == 'date') {
		                if (current.formatoptions && current.formatoptions.srcformat) {
		                    srcformat = current.formatoptions.srcformat;
		                } else {
		                    srcformat = $.jgrid.formatter.date.srcformat;
		                }
		                if (this.formatoptions && current.formatoptions.newformat) {
		                    newformat = current.formatoptions.newformat;
		                } else {
		                    newformat = $.jgrid.formatter.date.newformat;
		                }
		            } else {
		                srcformat = newformat = current.datefmt || "Y-m-d";
		            }
		            cmtypes[current.name] = { "stype": sorttype, "srcfmt": srcformat, "newfmt": newformat };
		        } else {
		            cmtypes[current.name] = { "stype": sorttype, "srcfmt": '', "newfmt": '' };
		        }
		        if (ts.p.grouping && current.name == grpview.groupField[0]) {
		            var grindex = current.name;
		            if (typeof current.index != 'undefined') {
		                grindex = current.index;
		            }
		            grtypes[0] = cmtypes[grindex];
		            grindexes.push(grindex);
		        }
		        if (!fndsort && (current.index == ts.p.sortname || current.name == ts.p.sortname)) {
		            st = current.name; // ???
		            fndsort = true;
		        }
		    }
		...
		td, ptr, tbody, imgs, iac = "", idc = "";
            ...
            for (i = 0; i < this.p.colNames.length; i++) {
                var tooltip = ts.p.headertitles ? (" title=\"" + $.jgrid.stripTitleHtml(ts.p.colNames[i]) + "\"") : "";
		...
		firstr = "<tr class='jqgfirstrow' role='row' style='height:auto'>";
            ...
            $("th", thr).each(function (j) {
                ...
                if (e.startHasBeenHandled) return;

                if ($(e.target).closest("th>span.ui-jqgrid-resize").length != 1) { return; }
                var ci = getColumnHeaderIndex(this);
                if (ts.p.forceFit === true) { ts.p.nv = nextVisible(ci); }
                grid.dragStart(ci, e, getOffset(ci));
                e.startHasBeenHandled = true;
                return false;
            }).click(function (e) {
		...
		.on('mousemove touchmove', function (e) {
		    if (e.moveHasBeenHandled) return;

		    if (grid.resizing) { grid.dragMove(e); return false; }
		    e.moveHasBeenHandled = true;
		});
    ...
    $.jgrid.extend({
                    ...
                    try {
                        $(this.p.colModel).each(function (i) {
                            ...
                            if (data[nm] !== undefined) {
                                ...
                                title = this.title ? { "title": $.jgrid.stripTitleHtml(vl)} : {};
                                if (t.p.treeGrid === true && nm == t.p.ExpandColumn) {
        ...
        showHideCol: function (colname, show) {
            return this.each(function () {
                // Fix for Chrome < version 19
                //var $t = this, fndh = false, brd = $.browser.webkit || $.browser.safari ? 0 : $t.p.cellLayout, cw;
                var $t = this, fndh = false, brd = ($.browser.webkit || $.browser.safari) && parseFloat($.browser.version) < 536.11 ? 0 : $t.p.cellLayout, cw;
                if (!$t.grid) { return; }
        ...
        setGridWidth: function (nwidth, shrink) {
            return this.each(function () {
                if (!this.grid) { return; }
                // fix for Chrome < ver 19
                /*var $t = this, cw,
                initwidth = 0, brd = $.browser.webkit || $.browser.safari ? 0 : $t.p.cellLayout, lvc, vc = 0, hs = false, scw = $t.p.scrollOffset, aw, gw = 0,
                cl = 0, cr;*/
                var $t = this, cw,
			initwidth = 0, brd = ($.browser.webkit || $.browser.safari) && parseFloat($.browser.version) < 536.11 ? 0 : $t.p.cellLayout, lvc, vc = 0, hs = false, scw = $t.p.scrollOffset, aw, gw = 0,
			cl = 0, cr;
                ...
                if (pos >= 0) {
                    ...
                    if (ind) {
                        ...
                        if (nData !== "" || forceupd === true) {
                            ...
                            title = $t.p.colModel[pos].title ? { "title": $.jgrid.stripTitleHtml(v)} : {};
...
$.jgrid.extend({
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			//if($t.p.subGrid === true || $t.p.sortable || $t.p.scroll || $t.p.grouping )    
			{
				return;
			}
			if($t.p.rownumbers) { i++; }
...
$.jgrid.extend({
		...
		var result = [];
			...
			switch ($t.p.treeGridModel) {
				case 'nested' :
					...
					levelc = $t.p.treeReader.level_field,
					lft = parseInt(rc[lftc],10), rgt = parseInt(rc[rgtc],10), level = parseInt(rc[levelc],10);
					$(this[0].p.data).each(function(){
						if(parseInt(this[0][levelc],10) === level+1 && parseInt(this[0][lftc],10) > lft && parseInt(this[0][rgtc],10) < rgt) {
							result.push(this[0]);
						}
					});
					break;
				case 'adjacency' :
					var parent_id = $t.p.treeReader.parent_id_field,
					dtid = $t.p.localReader.id;
				    var data = this[0].p.data;
				    for (var di = 0; di < data.length; di++) {
				        if (data[di][parent_id] == rc[dtid])
				            result.push(data[di]);				            				      
				    }
					break;
			}
		...
		return this.each(function(){
			...
			if(!rc[expanded]) {
				...
				if( $(this).jqGrid("isNodeLoaded",this.p.data[position]) ) {
					...
					rc[loaded] = true;
				    $(this).setSelection(id, true, null);
					if(this.p.treeGridModel == 'nested') {
	...
	SortTree : function( sortname, newDir, st, datefmt) {//HD got rid of use of $.each, it was really slowing us down
	    var df = this[0];
	    if(!df.grid || !df.p.treeGrid) {return;}
	    var i, len,
		rec, records = [], $t = df, query, roots,
		rt = this.jqGrid("getRootNodes");
		// Sorting roots
		query = $.jgrid.from(rt);
		query.orderBy(sortname,newDir,st, datefmt);
		roots = query.select();	    
        var collectChildrenFn = $.jgrid.getAccessor($.fn.jqGrid, "collectChildrenSortTree");
		...
		for (i = 0, len = roots.length; i < len; i++) {
		    ...
		    args[0] = records;
		    args[1] = rec;
		    collectChildrenFn.apply(this, args);
		}
	    var idprop = $t.p.localReader.id;
	    var subtree = $('#' + $.jgrid.jqID($t.p.id) + ' tbody');
        for (i=0; i < records.length; i++) {
            var current = records[i];
            var id = current[idprop]; 
            //$('#' + $.jgrid.jqID($t.p.id) + ' tbody tr:eq('+i+')').after($('tr#'+$.jgrid.jqID(id),$t.grid.bDiv));  //HD: this is the bottle-neck
            subtree.find('tr:eq(' + i + ')').after($('tr#' + $.jgrid.jqID(id), $t.grid.bDiv));                        
        }
      query = null;roots=null;records=null;
	},
	collectChildrenSortTree : function(records, rec, sortname, newDir,st, datefmt) { //HD got rid of use of $.each no need and it was really slowing us down
	    var df = this[0];
	    if(!df.grid || !df.p.treeGrid) {return;}
            var i, len,
