var guidely = (function () {
    return {
        ...
        , start: function (options) {
            ...
            if (that._options.welcome && !options.skipWelcome) {
                that.welcome();
                return;
            }
        ...
        , welcome: function () {
            ...
            pad.append('<h3>' + that._options.welcomeTitle + '</h3>');
            pad.append(that._options.welcomeText);

            controls = $('<div>', {
                'class': 'guidely-controls modal-footer'
            }).appendTo(popup);
            ...
            close = $('<a>', {
                'href': 'javascript:;'
                , 'class': 'guidely-close-trigger close'
                , 'html': '&times;'
        ...
        , _createGuide: function (id, options) {
            ...
            pad = $('<div>', {
                'class': 'guidely-guide-pad modal-body'
            }).appendTo(popup);

            if (options.title !== '' && options.title !== undefined) {
                pad.append('<h3>' + options.title + '</h3>');
            }
        ...
        , _getAnchorCoords: function (options) {
            var coords, offsetMap, anchor, xOffset, yOffset;
            coords = this._getCoords(options);
            anchor = options.anchor.replace('-', '_');
            yOffset = 8;
            xOffset = 26;
            offsetMap = {
                top_left: [coords.top - yOffset, coords.left - xOffset]
                , top_right: [coords.top - yOffset, (coords.left + coords.width) - xOffset]
                , top_middle: [coords.top - yOffset, coords.left + (coords.width / 2) - xOffset]
                , middle_left: [coords.top + (coords.height / 2) - yOffset, coords.left - xOffset]
                , middle_middle: [coords.top + (coords.height / 2) - yOffset, coords.left + (coords.width / 2) - xOffset]
                , middle_right: [coords.top + (coords.height / 2) - yOffset, coords.left + coords.width - xOffset]
                , bottom_right: [coords.top + coords.height - yOffset, (coords.left + coords.width) - xOffset]
                , bottom_middle: [coords.top + coords.height - yOffset, coords.left + (coords.width / 2) - xOffset]
                , bottom_left: [coords.top + coords.height - yOffset, coords.left - xOffset]
            };
        ...
        , _clear: function () {
            this._guides = [];
            $(".guidely-number, .guidely-guide, .guidely-overlay, #guidely-top").off().remove();
            $(window).off('resize.guidely');
        }
